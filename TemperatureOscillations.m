function [Difference] = TemperatureOscillations(filename, PdfFileName,...
    XlsFileName, R, dRdT, Uw, U3w, ln2w, n, save_pdf, save_png, resolution)

% function [DTaverage] = TemperatureOscillations(filename, PdfFileName,
% XlsFileName, R, dRdT, Uw, U3w, ln2w, n)
%   DTaverage:
%   
...
%   dRdT: Derivative of the resistance versus the temperature. Output of
%   the function dRdT. Uw: Applied voltage to the heater line (with the
%   frequency w). Output of the function PowerResistance. U3w: Measured
%   third harmonic of the viltage along the heater line. Output of the
%   function ThirdHarmonicOfVoltage. ln2w: Calculated logarithm of the
%   frequencies used during the measurement. Output of the function
%   ThirdHarmonicOfVoltage. n: Number of points taken to calculate the
%   average temperature
%       oscillations.
%
...
for i=1:L
    ...
    DT_corrected(:,i) = DT(:,i)*PixelWidth(i);
end
...
Cela(2:endValue+1,6:5+End) = num2cell(Difference);
% Cela(endValue+2,6) = cellstr('Average temperature oscillations');
% Cela(endValue+3,6:5+End) = num2cell(DTaverage);

     range = sprintf('A16:%s%0.f', char(65+5+End-1), endValue+2+16);
